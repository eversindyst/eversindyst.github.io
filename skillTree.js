	...
	var skillTree = {
		...
		mk3:"intl:1:Intelligence,mana:5:Mana,stam:10:Stamina,wis:1:Wisdom,intl:3:Bigger Brains,mreg:5:Mana Regen,intl:1:Intelligence,spdmg:3:Spell Power,res:5:Resist,mintl:3:Mental Fortitude,intl:1:Intelligence,wis:1:Wisdom,end:2:Endurance,hp:5:Life,spdmg:5:Unstable Energy,fdmg:2:Fire Damage,cldmg:2:Cold Damage,sdmg:2:Shock Damage,mana:10:Mana,mwis:3:Problem Solver,intl:1:Intelligence,mreg:5:Mana Regen,spdmg:3:Spell Power,intl:2:Intelligence,cchnc:3:Elemental Aggression,cdmg:20:Crit Damage,dex:2:Dexterity,intl:2:Intelligence,wis:2:Wisdom,mmana:10:Mind Drinker,intl:2:Intelligence,wis:2:Wisdom,dex:2:Dexterity,intl:2:Intelligence,spdmg:5:Cataclysm,fdmg:3:Fire Damage,cldmg:3:Cold Damage,sdmg:3:Shock Damage,intl:2:Intelligence,mintl:4:Natures Gift,hp:10:Life,mana:10:Life,mreg:5:Mana Regen,res:10:Resist,wis:5:Nirvana,intl:2:Intelligence,dex:2:Dexterity,cchnc:1:Crit Chance,cdmg:10:Crit Damage,spdmg:10:Natural Destruction,intl:2:Intelligence,wis:2:Wisdom,mana:10:Mana,mreg:5:Mana Regen,res:15:Spirit Form,intl:2:Intelligence,wis:2:Wisdom,spdmg:3:Spell Power,end:3:Endurance,mk3m:0:Ascendant,intl:2:Intelligence,hp:10:Life,mana:10:Mana,wis:2:Wisdom,intl:5:Primal Spirit"
	}
	...
	function skillTreeSelected(x, cv){
		...
		selectedSkills = selSk1+";"+selSk2+";"+selSk3;
		mc.skillsRemain -= 1;
		skillTreeEffect(c, v);
	...
	function loadSavedSkills(){
		if(selectedSkills.length > 0){
			...
			for(var y =0; y < 3; y++){
				for(var x = 0; x < ss[y]; x++){
					freeSkillTree(y);
				}
			}
		}
	}
	function freeSkillTree(x){
		var upgStr;
		if(x == 0){
			var index = currTree1.indexOf(",");
			if(index != -1){
				upgStr = currTree1.substr(0,index);
				currTree1 = currTree1.slice(index+1);
			}
			else{
				upgStr = currTree1;
			...
			if(x == 1){
				var index = currTree2.indexOf(",");
				if(index != -1){
					upgStr = currTree2.substr(0,index);
					currTree2 = currTree2.slice(index+1);
				}
				else{
					upgStr = currTree2;
				...
				if(x == 2){
					var index = currTree3.indexOf(",");
					if(index != -1){
						upgStr = currTree3.substr(0,index);
						currTree3 = currTree3.slice(index+1);
					}
					else{
						upgStr = currTree3;
